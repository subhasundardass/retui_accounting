...
// The Ledger_GroupFunc type is an adapter to allow the use of ordinary
// function as Ledger_Group mutator.
type Ledger_GroupFunc func(context.Context, *ent.LedgerGroupMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f Ledger_GroupFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.LedgerGroupMutation); ok {
		return f(ctx, mv)
	}
