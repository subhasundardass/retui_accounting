...
import (
	...
	"github.com/subhasundardass/retui/module/payment"
...
func NewFormComponent(ctx *appctx.AppContext) *FormComponent {
	return &FormComponent{
		controller: payment.NewController(ctx),
		state: payment.FormState{
			Lines: []payment.PartyLine{{}},
		},
	}
}

func (c *FormComponent) bindKeys(form *retui.Form[payment.FormState]) {
...
func (c *FormComponent) Payment(ctx *appctx.AppContext) retui.Element {
...
func (c *FormComponent) new(ctx *appctx.AppContext) retui.Element {
	...
	return components.Panel().
		Header(retui.Box(
			...
			retui.NewStyle(),
			retui.Text("Payment Entry", retui.NewStyle().Bold(true)),
...
func (c *FormComponent) buildHead(ctx *appctx.AppContext, form *retui.Form[payment.FormState]) retui.Element {
	v := form.Values()
	...
	slNo := retui.Box(
		...
		retui.Box(
			...
			retui.NewStyle(),
			retui.Text("Payment No", retui.NewStyle()),
		),
	...
	pymtAccount := retui.Box(
		...
		retui.Box(
			...
			retui.NewStyle(),
			retui.Text("Paid By", retui.NewStyle()),
		),
		widgets.CashBankDropdown(
			ctx, "pymyAccount", strconv.Itoa(v.PymtAccount), 40, v.FocusIndex == 4,
			func(id, value string) {
				if err := form.SetField("PymtAccount", util.StringToInt(value, 0)); err != nil {
					retui.Debugf("SetField error: %v", err)
				}
			},
		),
...
// -- Party Section
func (c *FormComponent) buildPartyRow(ctx *appctx.AppContext, form *retui.Form[payment.FormState]) retui.Element {
...
func (c *FormComponent) lineItemRows(ctx *appctx.AppContext, form *retui.Form[payment.FormState]) []retui.Element {
...
// - row
func (c *FormComponent) lineRow(ctx *appctx.AppContext, form *retui.Form[payment.FormState], i int) retui.Element {
	...
	setLine := func(mutate func(*payment.PartyLine)) {
		nv := form.Values()
		line := nv.Lines[i]
		mutate(&line)
		nv.Lines[i] = line
	...
	partyLedger := retui.Box(
		...
		widgets.LedgerComponent(
			...
			v.FocusIndex == base,
			func(id, value string) {
				setLine(func(l *payment.PartyLine) { l.Ledger = util.StringToInt(value, 0) })
			},
		),
	)
	partyRemarks := retui.Box(
		...
		components.TextInput().
			...
			Width(65).
			OnChange(func(id, value string) {
				setLine(func(l *payment.PartyLine) { l.Remarks = value })
			}).
			Render(),
	)
...
// --Save
func (c *FormComponent) save(form *retui.Form[payment.FormState]) {

	state := form.Values()

	if err := payment.ValidateForm(state); err != nil {
		components.ShowError(err.Error())
		return // validation failed — don't reset, keep the user's input
	}

	mode := payment.ModeCreate
	id := 0
	if c.editing {
		mode = payment.ModeUpdate
		id = c.editID
	}
	...
	c.editing = false
	c.editID = 0
	c.state = payment.FormState{Lines: []payment.PartyLine{{}}}
